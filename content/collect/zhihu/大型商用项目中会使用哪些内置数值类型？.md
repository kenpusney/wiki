...
同样的比如要保存精准时间点有[ISO 8601 Datetime](http://en.wikipedia.org/wiki/ISO_8601)类型，同时也有[timestamp](http://en.wikipedia.org/wiki/Unix_timestamp)这种东西，只要一个整数就可以了。权衡利弊的话，timestamp对空间占用较小，方便存储和传输，反过来问题就是没有Datetime那样灵活，而且还可能会出现[Year 2038 problem](http://en.wikipedia.org/wiki/Y2038)。
