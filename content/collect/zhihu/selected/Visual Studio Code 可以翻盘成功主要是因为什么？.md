...
1. 抛开后期各种扩展性不谈，你看VSCode最早抓住的点：性能。这恰好就是那段时间这么多编辑器无论如何都没解决掉的一个关键问题，VSCode给搞定了。也一样，靠技术实力嘛，微软这么大个公司，处理有限性能下大文本即时渲染的方案肯定早就有挺多套了。你去看下，少有几个应用软件会在自己的博客上探讨数据结构（不是数据模型）这种非常底层的概念，vscode的博客翻一翻你能看到不少。
