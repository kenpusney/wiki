...
类比硬件的概念。使用草图而不是具体的视觉元素来展示和讨论关键组件和他们的链接。

基本包含三部分：

1. 位置 Places - 界面、对话框、菜单、弹框
2. 功能可见性 Affordances - 用户可以交互的东西，比如按钮或者字段
3. 连接线 Connection lines - 通过affordance引导用户在place之间跳转的链接

使用文字而非图片，关键是为了找出组件和之间的链接。

### 粗马克笔草图

不希望浪费时间在线框图和不必要的保真度（fidelity）上。粗马克笔草图非常难添加细节，可以避免这个东西。

### 元素是产出物

Shaping阶段产出的是能够准确表述我们需求的基本元素（而非面面俱到的细节）。

### 留给设计师的空间

无论你说什么，任何mockup上的细节都可能影响别人怎么想——尤其是当你处于高位时。

因此使用正确的“抽象层级”不仅保证了我们能够以正确的速度前进，还能为后续的创作流出足够的空间。

这并不是个spec，更像是一个游戏的边界和规则。

### 暂时还不能交付

- 从一个模糊的想法已经变成了具体的元素了，但是依然太粗略（rough）
- 除了提出解决问题的方法以外，很多问题还都是未知的
- 下一步时压力测试和去风险化
...
- 未知是永远存在的
- 但是在Building过程之前，有很多问题可以在betting之前提前提出并解决的
...
- 推荐异步沟通 —— 可以最大化地让每个人使用自己可控的时间来干真正的事情
- 保持这个圆桌足够小并且产出高效
- 如果不方便做到，那就做一个快速的线上宣讲
